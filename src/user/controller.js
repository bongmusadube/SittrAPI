...
const getCaregiverIncome = async (req, res) => {
  ...
  SELECT
        ...
        SUM(total_amount - total_amount * 0.1) AS "TotalAmount"
      FROM (
        SELECT
    ...
    const result = await pool.query(query, [caregiverEmail]);

    // Map the results to the desired JSON format
    const monthlyTotalAmounts = result.rows.map(row => {
      return {
        Month: row.Month,
        TotalAmount: row.TotalAmount.toFixed(2) // Format the total amount as desired
      };
    });

    res.json(monthlyTotalAmounts);
  } catch (error) {
...
const DeclineCaregiver = async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO notapproved (careid, comment)
      VALUES ($1, $2)
      RETURNING *;`;

    const values = [careId, comment];
...
const SendEmail = async (req, res) => {
  try {
    const { senderEmail, recipientEmail, ccEmail, subject, message } = req.body;

    // Check if sender email exists in the caregivers or admin table
    ...
    // If sender email is not found in caregivers table, check in the admin table
    if (senderResult.rows.length === 0) {
      const adminExistsQuery = `
        ...
        WHERE email = $1
      `;
      const adminExistsValues = [senderEmail];
      const adminResult = await pool.query(adminExistsQuery, adminExistsValues);
    ...
    // Check if recipient email exists in the caregivers or admin table
    ...
    // If recipient email is not found in caregivers table, check in the admin table
    if (recipientResult.rows.length === 0) {
      const adminRecipientExistsQuery = `
        SELECT id
        FROM admin
        WHERE email = $1
      `;
      const adminRecipientExistsValues = [recipientEmail];
      const adminRecipientResult = await pool.query(adminRecipientExistsQuery, adminRecipientExistsValues);

      if (adminRecipientResult.rows.length === 0) {
    ...
    const insertEmailQuery = `
      INSERT INTO emails (sender_email, recipient_email, cc_email, subject, message)
      VALUES ($1, $2, $3, $4, $5)
    `;
    const insertEmailValues = [senderEmail, recipientEmail, ccEmail, subject, message];
    await pool.query(insertEmailQuery, insertEmailValues);

    // Send the email to the CC recipient if CC email is provided
    if (ccEmail) {
      const insertCCEmailQuery = `
        INSERT INTO emails (sender_email, recipient_email, cc_email, subject, message)
        VALUES ($1, $2, $3, $4, $5)
      `;
      const insertCCEmailValues = [senderEmail, recipientEmail, ccEmail, subject, message];
      await pool.query(insertCCEmailQuery, insertCCEmailValues);
    }
...
const GetSentEmails = async (req, res) => {
  try {
    ...
    const query = `
      SELECT id,sender_email,recipient_email,subject, message,sent_at
      FROM emails
...
const GetReceivedEmails = async (req, res) => {
  try {
    ...
    const query = `
      SELECT id,sender_email,recipient_email,subject, message,sent_at
      FROM emails
      WHERE recipient_email = $1 OR cc_email = $1
      ORDER BY sent_at DESC
    `;
...
const GetDeclined = async (req, res) => {
  try {
